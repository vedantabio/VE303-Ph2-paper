...
library(here)
mainDir <- here("early_recurrence_model_importance")
...
pred_time_set <-  readRDS(here("early_recurrence_model_performance/pred_time_set_list_all.rds"))
...
saveRDS(metric_time_list,paste0(results_folder,"/","Model_performance.rds"))
saveRDS(freq_list,paste0(results_folder,"/","Recur_Freq.rds"))
metric_time_list <- readRDS(paste0(results_folder,"/","Model_performance.rds"))
freq_list <-  readRDS(paste0(results_folder,"/","Recur_Freq.rds"))
...
pdf(paste0(results_folder,"/RF_Early_Recurrence_Model_performance.pdf"),width = 8,height = 6)
print(bar_p)
dev.off()
...
# Make a data list across all times for each data set
time_data_list <-  readRDS(here("early_recurrence_model_performance/time_data_list.rds"))
...
# Recurrence data
# Filter Early recurrers only:
time_recur_dt <-  unique(sm_dt[,c("Subject.Number","rec.diagnosis.Wk8","rec.day.in.treatment","TRT.norm")]) %>% 
  mutate(Recurrence =  ifelse(rec.day.in.treatment > 14,"Late","Early"))
time_recur_dt$Recurrence[time_recur_dt$rec.diagnosis.Wk8 == "Good"] <- "NR"
late_recur_sub <-  time_recur_dt$Subject.Number[time_recur_dt$Recurrence == "Late"]
...
rec_dt <-  rec_dt %>% filter(!Subject.Number %in% late_recur_sub)
...
# Import prediction results
time_slots <- c("Screening","Day 1","Day 7")
...
time_slots <-  names(p_imp_list)
for(sel_time in time_slots){ 
  data_list <- p_imp_list[[sel_time]]
  # Remove NULL elements  
  data_list <-  purrr::compact(data_list)
  # Now loop over each data set:
  dt_set <-  names(data_list)[2]
   pdf(paste0(results_folder,"/",sel_time,"_Variable_imp_RF.pdf"), width = 15, height = 8)
  for(dt_set in names(data_list)){
      p_imp <-  p_imp_list[[sel_time]][[dt_set]]
        #p_abun_list[[sel_time]][[dt_set]] <- bar_p
      print(p_imp )
...
# screen_imp_dt$variable <- gsub("_Screening","",screen_imp_dt$variable)
# screen_imp_dt$variable <- gsub("\\.\\."," ",screen_imp_dt$variable)
# screen_imp_dt$variable <- gsub("\\."," ",screen_imp_dt$variable)
# screen_imp_dt$variable <- gsub("^X","",screen_imp_dt$variable)
# 
# screen_imp_dt$variable <- gsub("total_abx_days_prescreen","Total Days of Abx",screen_imp_dt$variable)
# screen_imp_dt$variable <- gsub("n_pretreatment_events","N Abx Episodes",screen_imp_dt$variable)
...
screen_imp_dt$clean_names <-  gsub("^X2 ","2-",screen_imp_dt$clean_names)
screen_imp_dt$clean_names <-  gsub("_MP.*","",screen_imp_dt$clean_names)
...
screen_imp_dt$clean_names[screen_imp_dt$clean_names == "IL 17E IL 25"] <- "IL-17E/IL-25"
screen_imp_dt$clean_names[screen_imp_dt$clean_names == "ENA 78"] <- "ENA-78"
screen_imp_dt$clean_names[screen_imp_dt$clean_names == "IL 27"] <- "IL-27"
screen_imp_dt$clean_names[screen_imp_dt$clean_names == "PDGF AB BB"] <- "PDGF-AB-BB"
...
p_imp <- ggplot( screen_imp_dt, aes(y= reorder_within(clean_names,Mean_imp,dataset), x=Mean_imp))+
  ...
  geom_errorbar(aes(xmin=Mean_imp-se, xmax=Mean_imp+se),
                width=.2,colour="black", alpha=0.9) +
  facet_rep_wrap(~dataset,scales = "free",repeat.tick.labels = T)+
  scale_alpha_continuous(limits = c(0,1),range = c(0,1))+
  scale_fill_manual(values = col_R) +
...
# pdf(paste0(results_folder,"/","Screening_imp_RF.pdf"), width = 15, height = 10)
# print(p_imp)
# dev.off()
library(Cairo)
cairo_pdf(paste(results_folder,"/Screening_imp_RF.pdf",sep=""),width = 15, height = 10)
print(p_imp)
dev.off()
