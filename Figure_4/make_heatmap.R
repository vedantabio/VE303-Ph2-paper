...
imp_dt_list <-  readRDS(here("recurrence_model_importance/Heatmap_imp_list.rds"))

sum_imp_final_dt <-  do.call("rbind",imp_dt_list)

# Import Performance of the models:
perf_dt <-  read.csv(here("recurrence_model_importance/Performance_colonization.csv"))
head(perf_dt)
...
mat_logic <- mat_logic[,c("Screening","Day 1","Day 7","Day 14")]
...
# split_rows <-  factor(split_rows,levels = c("Clinical factors","Vancomycin","SCFA","BA","Cytokines","Order","Class","Genus","Species"))
