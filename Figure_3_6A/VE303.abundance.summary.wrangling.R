...
ve303.abund.mean.det <- ve303.abund.sum.coh.det %>% 
  ...
  arrange(TRT.norm)
#write.csv(ve303.abund.mean.det, file = here(results, paste(Sys.Date(), "ve303_abundance_mean_summary_det.csv", sep = " ")), row.names = FALSE)

ve303.abund.med.det <- ve303.abund.sum.coh.det %>% 
  select(organism, Visit.Name.Norm, TRT.norm, median) %>% 
  spread(Visit.Name.Norm, median) %>% 
  arrange(TRT.norm)
#write.csv(ve303.abund.med.det, file = here(results, paste(Sys.Date(), "ve303_abundance_median_summary_det.csv", sep = " ")), row.names = FALSE)
...
#write.csv(ve303.abund.timepoint.total.mean.det, file = here(results, paste(Sys.Date(), "ve303_abundance_total_mean_summary_det.csv", sep = " ")), row.names = FALSE)
...
ve303.abund.timepoint.total.median.det <- ve303.abund.subj.total.time.det %>% 
  ...
  spread(Visit.Name.Norm, est_relative_abundance_panel)
#write.csv(ve303.abund.timepoint.total.median.det, file = here(results, paste(Sys.Date(), "ve303_abundance_total_median_summary.csv", sep = " ")), row.names = FALSE)
...
# write.csv(ve303.ABS.abund.timepoint.total.mean.det.alt, file = here(results, paste(Sys.Date(), "ve303_ABS_abundance_total_mean_summary_det.csv", sep = " ")), row.names = FALSE)
