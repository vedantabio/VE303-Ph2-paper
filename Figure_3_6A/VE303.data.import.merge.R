...
print("Analysis_78 = yes or no: if yes then script will eclude HD Good subject 100501")
print(paste0("Analysis_78 = ",analysis_78))
print(paste0("N samples in Merged Sequencing Manifest: ",dnameta$MGS.Sample.ID %>% unique() %>% length()))
print(paste0("N subjects in Merged Sequencing Manifest: ", dnameta$Subject.Number %>% unique() %>% length() ))
...
### 2 Samples present in VE303 processed files from OC but absent in the Sequencing manifest  ##

print(paste0("N Samples in VE303 OC file missing from the Sequencing manifest = ",  length(diff_S)))
...
print(paste0("Number of Unique Visit names :", ve303.dnameta$Visit.Name.Norm %>% unique() %>% length()))

########################################################################################


## Wrangling from Stool Processing Record input metadata files 
