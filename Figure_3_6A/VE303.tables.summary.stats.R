...
#write.csv(ve303.colonize.det.stats.mean, file = here(results, paste(Sys.Date(), "VE303_colonization_detected_notdetected_total_mean.csv", sep = " ")), row.names = FALSE)
...
#write.csv(ve303.colonize.det.recur.stats.mean, file = here(results, paste(Sys.Date(), "VE303_colonization_detected_notdetected_total_mean_by recurrence.csv", sep = " ")), row.names = FALSE)
...
#write.csv(ve303.colonize.sum.bystrain.tp.det, file = here(results, paste(Sys.Date(), "VE303_colonization_detected_notdetected_total_prev_by_strain.csv", sep = " ")), row.names = FALSE)
...
ve303.abund.mean <- ve303.abund.sum %>% 
  ...
  spread(Visit.Name.Norm, mean) 
#write.csv(ve303.abund.mean, file = here(results, paste(Sys.Date(), "ve303_abundance_mean_summary.csv", sep = " ")), row.names = FALSE)

ve303.abund.med <- ve303.abund.sum %>% 
  select(organism, Visit.Name.Norm, TRT.norm, median) %>% 
  spread(Visit.Name.Norm, median) %>% 
  arrange(TRT.norm)
#write.csv(ve303.abund.med, file = here(results, paste(Sys.Date(), "ve303_abundance_median_summary.csv", sep = " ")), row.names = FALSE)
...
#write.csv(ve303.abund.timepoint.total.mean, file = here(results, paste(Sys.Date(), "ve303_abundance_total_mean_summary.csv", sep = " ")), row.names = FALSE)

ve303.abund.timepoint.total.median <- ve303.abund.subj.total.time %>% 
  group_by(TRT.norm, Visit.Name.Norm) %>% 
  summarise_at(vars(est_relative_abundance_panel), funs(median)) %>%  
  spread(Visit.Name.Norm, est_relative_abundance_panel)
#write.csv(ve303.abund.timepoint.total.median, file = here(results, paste(Sys.Date(), 
#                                                                         "ve303_abundance_total_median_summary.csv", sep = " ")), row.names = FALSE)
